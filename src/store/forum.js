  ...
  actions: {
    ...
    async createforumBranch({ commit, getters }, payload) {
      ...
      try {
        ...
        const forumBranch = await firebase
          .database()
          .ref("forumBranch")
          .push(newForumBrach);
    ...
    async fetchForumBranch({ commit }) {
      ...
      try {
        const fbForumBranch = await firebase
          .database()
          .ref("forumBranch")
          .once("value");
        ...
        if (forumBranchs)
          Object.keys(forumBranchs).forEach((key) => {
            ...
            resultForumBranch.push(
              new ForumBrach(
                forumBranch.nameBranch,
                forumBranch.dateCreate,
                forumBranch.ownerId,
                key,
              ),
            );
  ...
  getters: {
    ...
    forumBranchId(state) {
      return (forumBranchId) => {
        return state.forumBranch.find(
          (forumBranch) => forumBranch.id === forumBranchId,
        );
      };
    },
    ...
    branchMessagesId(state) {
      return (branchMessagesId) => {
        return state.branch.find(
          (branch) => branch.idForumBranch === branchMessagesId,
        );
      };
    },
  },
