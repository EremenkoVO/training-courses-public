...
export default {
  ...
  actions: {
    ...
    async createArticle({ commit }, payload) {
      ...
      try {
        const newArticle = new Article(payload.nameArticle, payload.pdfFile);
        ...
        const fileData = await firebase
          .storage()
          .ref(`Articles/${article.key}.${pdfExt}`)
          .put(payload.pdfFile);
        const pdfFile = await firebase
          .storage()
          .ref()
    ...
    async fetchArticles({ commit }) {
      ...
      try {
        const fbArticles = await firebase
          .database()
          .ref("article")
          .once("value");
